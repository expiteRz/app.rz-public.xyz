export default function AngleDown({ size, className }: { size: number; className?: string }) {
  return (
    <svg class={className} xmlns="http://www.w3.org/2000/svg" height={size} viewBox="0 0 448 512">
      <path
        fill="currentColor"
        d="M201.4 374.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 306.7L86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"
      />
    </svg>
  );
}
